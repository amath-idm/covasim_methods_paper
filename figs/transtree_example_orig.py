...
ax.axis('off')
lcol = colors[0]
pl.plot(np.nan, np.nan, 'o', c=suscol, markersize=msize, **plargs, label='Never infected')
...
for day in range(sim.npts):
    for i,key in enumerate(sims.keys()):
        pl.sca(axes[i])
        sim = sims[key]
        pl.title(f'{labels[i]}; total infections: {sim.results["cum_infections"][day]:0.0f}')
        pl.xlim([0, sim.npts])
        pl.ylim([0, sim.n])
        pl.xlabel('Day of infection')
        pl.ylabel('Person')
        flist = F[key][day]
        tlist = T[key][day]
        dlist = D[key][day]
...
sc.toc(tstart)
pl.savefig('transtree_example.png')